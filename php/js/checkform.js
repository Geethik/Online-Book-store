function validate(userForm) {
	...
	if(userForm.password.value != userForm.repassword.value)
	{
 	...
 	var div=document.getElementById("usrmsg1");
	div.style.color="red";
	if(div.hasChildNodes())
	{
		div.removeChild(div.firstChild);
	...
	var div=document.getElementById("usrmsg2");
	div.style.color="red";
	if(div.hasChildNodes())
	{
		div.removeChild(div.firstChild);
	...
	var div=document.getElementById("streetnomsg");
	div.style.color="red";
	if(div.hasChildNodes())
	{
		div.removeChild(div.firstChild);
	...
	if(userForm.complete_name.value.length ==0)
	{
		div.appendChild(document.createTextNode("Street Number cannot be blank"));
		userForm.complete_name.focus();
		return false;
	}
	var div=document.getElementById("streetnamemsg");
	div.style.color="red";
	if(div.hasChildNodes())
	{
		div.removeChild(div.firstChild);
	...
	if(userForm.complete_name.value.length ==0)
	{
		div.appendChild(document.createTextNode("Street Name cannot be blank"));
		userForm.complete_name.focus();
		return false;
	}
	return true;
